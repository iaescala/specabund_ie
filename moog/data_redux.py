...
hash_blue=None, hash_red=None, synth_path_blue='', synth_path_red='', phot_dict=None,
dlam_dict=None, dlamfix=False):
    ...
    par_limits = {'teff': [3500., 8000.], 'logg': [0., 5.], 'feh': [-4.8, 0.],\
    'alphafe': [-0.8, 1.2], 'dlam': [1., 1.4]}
    ...
    def get_synth(teff, logg, feh, alphafe, dlam):
        ...
        if len(wb) > 0:
            try:
                resultb = read_synth.read_interp_synth(teff=teff,
                logg=logg, feh=feh, alphafe=alphafe, data_path=synth_path_blue,
                hash=hash_blue)
            except IOError:
                return None

            if resultb is not None:
                wvl_synth_blue, relflux_synth_blue = resultb
                wsynthb = np.where((wvl_synth_blue > wvl[0])&(wvl_synth_blue < wvl[-1]))[0]
            else: return None

        if len(wr) > 0:
            resultr = read_synth.read_interp_synth(teff=teff,
            logg=logg, feh=feh, alphafe=alphafe, data_path=synth_path_red,
            hash=hash_red, start=6300., sstop=9100.)

            if resultr is not None:
                wvl_synth_red, relflux_synth_red = resultr
...
cont_regions=None, sigma_l_red=5., cont_mask=None, grating='600ZD', zrest=0., npix=None,
maxiter=None):
    ...
    for i in range(len(wave)):
        ...
        if m <= k:
            ...
            sigma_l, sigma_u, npix, silent=False, maxiter=maxiter)
...
grating='', sigma_l=3., sigma_u=3., cont_mask=None, norm=None, npix=None, maxiter=None):
    ...
    plt.ylim(0.7, 1.1)
    plt.savefig('flux.refine_n2419c_'+str(npix)+'_k5_100.pdf')
    plt.close()
    """
