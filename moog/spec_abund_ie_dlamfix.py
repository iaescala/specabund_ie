...
dlam=1.2, synth_path_blue='', synth_path_red='', slitmask_path='', moogify_path='',
...
telluric_path='', object_id=None, wgood=False, m31disk=None):
    ...
    if m31disk is not None:
        ...
        if ~np.isnan(wchip).any():
            ...
            wvl = np.array([wvl0[int(wchip[0]):int(wchip[1])], wvl0[int(wchip[2]):int(wchip[3])]])
            spec = np.array([spec0[int(wchip[0]):int(wchip[1])], spec0[int(wchip[2]):int(wchip[3])]])
            ...
            if ~np.isnan(wb).all():
                ...
                wvl = np.array([wvl0[int(wchip[0]):int(wchip[1])]])
                spec = np.array([spec0[int(wchip[0]):int(wchip[1])]])
                ivar = np.array([ivar0[int(wchip[0]):int(wchip[1])]])

            if ~np.isnan(wr).all():

                wvl = np.array([wvl0[int(wchip[2]):int(wchip[3])]])
                spec = np.array([spec0[int(wchip[2]):int(wchip[3])]])
        ...
        flux_tell_flat, ivar_tell_flat = data_redux.telluric_correct(wvl, spec, ivar, xspec,
            grating=grating, telluric_path=telluric_path, m31disk=True)

        flux_tell = np.array( [ flux_tell_flat[:len(spec[0])], flux_tell_flat[len(spec[0]):] ] )
    ...
    while i < maxiter:
        ...
        sigma=sigma_teff, bounds=([3500., fehmin],[8000., 0.]), absolute_sigma=True,
    ...
    xteff, teffcontour = data_redux.check_fit(params_dict, perr_teff, wave_flat, flux_refine,
        ivar_refine, mask=feh_fit_mask, hash_red=hash_red, phot_dict=phot_dict,
    ...
    xfeh, fehcontour = data_redux.check_fit(params_dict, perr_feh, wave_flat, flux_refine,
        ivar_refine, mask=feh_fit_mask, hash_red=hash_red, dlamfix=True,
        hash_blue=hash_blue, synth_path_blue=synth_path_blue, synth_path_red=synth_path_red)

    xalphafe, alphafecontour = data_redux.check_fit(params_dict, perr_alphafe, wave_flat,
        flux_refine, ivar_refine, mask=alphafe_fit_mask, hash_red=hash_red, dlamfix=True,
        hash_blue=hash_blue, synth_path_blue=synth_path_blue, synth_path_red=synth_path_red)
