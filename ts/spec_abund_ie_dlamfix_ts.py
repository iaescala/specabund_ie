...
import data_redux_ts as data_redux
...
def spec_abund(slitmask_name=None, slit_str=None, slit_num=None, feh_def = -2., alphafe_def = 0.,
...
telluric_path='', object_id=None, wgood=False, m31disk=None):
    ...
    if m31disk is not None:
        ...
        if ~np.isnan(wchip).any():
            ...
            wave = np.array([wvl0[int(wchip[0]):int(wchip[1])], wvl0[int(wchip[2]):int(wchip[3])]])
            flux = np.array([spec0[int(wchip[0]):int(wchip[1])], spec0[int(wchip[2]):int(wchip[3])]])
            ...
            if ~np.isnan(wb).all():
                ...
                wave = np.array([wvl0[int(wchip[0]):int(wchip[1])]])
                flux = np.array([spec0[int(wchip[0]):int(wchip[1])]])
                ivar = np.array([ivar0[int(wchip[0]):int(wchip[1])]])

            if ~np.isnan(wr).all():

                wave = np.array([wvl0[int(wchip[2]):int(wchip[3])]])
                flux = np.array([spec0[int(wchip[2]):int(wchip[3])]])
    ...
    ### Temperature-gravity dependent spec regions for TS spectra
    q = 90 #90% of the most sensitive pixels

    #Teff-logg grid on which the masks are evaluated
    teff_mask_arr = np.arange(3000., 6500., 500.)
    logg_mask_arr = np.arange(0., 4.0, 0.5)

    teff_mask = int(teff_mask_arr[np.argmin(np.abs(teffphot - teff_mask_arr))])
    logg_mask = logg_mask_arr[np.argmin(np.abs(loggphot - logg_mask_arr))]

    print(teffphot, teff_mask, loggphot, logg_mask)

    feh_mask = data_redux.check_pkl_file_exists(f'mask_t{teff_mask}_logg{logg_mask}_fe_q{q}',
    file_path =f'{mask_path}/{grating}/')

    alphafe_mask = data_redux.check_pkl_file_exists(f'mask_t{teff_mask}_logg{logg_mask}_alpha_q{q}',
    file_path =f'{mask_path}/{grating}/')

    ### Combine them with the regions we know are difficult to model
    ...
    def construct_synth(wvl, teff_in, feh_in, alphafe_in, hash=None):
        """
    ...
    while i < maxiter:
        ...
        alphafe_thresh) and (np.abs(teff_best0 - teff0) < teff_thresh):
            ...
            print(teff_best0, feh_best0, alphafe_best1, flush=True)
